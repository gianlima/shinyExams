...
explanations[1] <- if (solutions[1]) "Ambas as distribuições têm um local semelhante." else
  paste("Distribuição ", c("A", "B")[(median(A) < median(B)) + 1],
    " tem em média valores superiores aos da distribuição ", 
    c("A", "B")[(median(A) >= median(B)) + 1], ".", sep = "")

outlier <- length(unlist(outlier)) > 0
questions[2] <- "Ambas as distribuições não contêm outliers."
solutions[2] <- !outlier
explanations[2] <- if (solutions[2])
  "Ambas as distribuições não têm observações que se desviam mais de 1,5 vezes a faixa interquartil da caixa." else "Existem observações que se desviam mais de 1,5 vezes a faixa interquartil da mediana."

questions[3] <- "A extensão na amostra A é claramente maior que em B."
solutions[3] <- IQR(A)/IQR(B) > 1.5
explanations[3] <- paste("A faixa interquartil na amostra A",
  ifelse(solutions[3], "é", "_não_ é"), "claramente maior do que em B.")

questions[4] <- "A assimetria  de ambas as amostras é semelhante."
...
explanations[4] <- if (solutions[4]) paste("A assimetria de ambas as distribuições é semelhante, ambas são",
    ...
    paste("A assimetria de ambas as distribuições é diferente. A amostra A é",
    ifelse(abs(SK(A)) < 0.2, "simétrica.", ifelse(SK(A) >= 0.2, "assimétrica à direita.", "assimétrica à esquerda.")),
    "A amostra B é", ifelse(abs(SK(B)) < 0.2, "simétrica.", ifelse(SK(B) >= 0.2, "assimétrica à direita.", "assimétrica à esquerda.")))
...
solutions[5] <- switch(j, abs(SK_i) < 0.2,
                       SK_i >= 0.2,
                       SK_i <= -0.2)
explanations[5] <- paste("Distribuição ", c("A", "B")[i], " is ",
...
Question
========
Na figura a seguir, as distribuições de uma variável dada por duas amostras (A e B) são representadas por boxplots paralelos. Quais das seguintes afirmações são corretas?  _(Observação: As afirmações são corretas ou erradas.)_
\
...
exname: Boxplots paralelos de uma mesma variável dada por duas amostras;;
